...
import numpy as np
# reading CSV file
dataInput = read_csv("D:\materials of JI\\4903robotic arm\\ansys working\\squarebotsuf\\4903\\ft_hexa1.csv")
dataOutput = "D:\materials of JI\\4903robotic arm\\ansys working\\ansys scripting\\output_hexa_bottomsurf_nodes.csv"
...
def judgement(x,y,dx,dy,da):
    ...
    if (y  > -h and y  < h) and (k * (x - 12.5) < y < -k * (x - 12.5)) and (-k * (x + 12.5) < y < k * (x + 12.5)):
        if (y1  < -h or y1  > h) or (k * (x1 - 12.5) > y1) or (y1 > -k * (x1 - 12.5)) or (-k * (x1 + 12.5) > y1) or (y1 > k * (x1 + 12.5)):
            return 1
    return 0
