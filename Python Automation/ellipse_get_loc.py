...
import numpy as np
# reading CSV file
dataInput = read_csv("D:\materials of JI\\4903robotic arm\\ansys working\\squarebotsuf\\4903\\ft_elli.csv")
dataOutput = "D:\materials of JI\\4903robotic arm\\ansys working\\ansys scripting\\ft_output_ellipse_bottomsurf_nodes.csv"
...
# width a, height b
input_displacement=[]
# this is the half the length of the side lying on the y-axis
a = 12.5
# this is the half the length of the side lying on the x-axis
b = 12.5
z_0 = 3.64
y_MAX = 12
z_MAX = 12
...
def judgement(x,y,dx,dy,da):
    x1 = x  *np.cos(da) - y  *np.sin(da) + dx
    y1 = x  *np.sin(da) + y  *np.cos(da) + dy
    D_f1_t = ((x1 - 7.5)**2 + y1**2 ) ** 0.5
    D_f2_t = ((x1 + 7.5)**2 + y1**2 ) ** 0.5
    D_f1 = ((x - 7.5)**2 + y**2 ) ** 0.5
    D_f2 = ((x + 7.5)**2 + y**2 ) ** 0.5
    if (D_f1_t + D_f2_t > 25) and (D_f1 + D_f2 < 25):
        return 1
    return 0
